...
const SignIn = ({modalIsOpen, toggleModal, setAuth}) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const fetchResponse = await fetch(`${BASE_URL}/api/signin`, settings);
  ...
  return (
    <Modal isOpen={modalIsOpen} onRequestClose={toggleModal} className="modal">
      <form className="modal__form" onSubmit={handleSubmit(onSubmit)}>
        <h2 className="modal__heading">Please enter an Email and a Password <FaPaw /> </h2>
        ...
        <input 
          ...
          ref={register({required: true})}
        />
        <span className="modal__form--check">Forgot Password? <Link to="/">Click Here</Link></span>
        <button className="modal__form--btn" type="submit">ENTER</button>
      </form>
